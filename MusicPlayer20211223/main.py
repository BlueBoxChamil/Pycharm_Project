...
class musicPlayer(QWidget):
    ...
    def __initialize(self):
        self.setWindowTitle('BlueBox Music Player')
        self.setWindowIcon(QIcon('E://BlueBox//BlueBox_file//Pycharm_Project//20211223//H-icon.ico'))
        self.songs_list = []
        self.song_formats = ['mp3', 'wav']
        if not os.path.isfile(os.getcwd() + '\\'  + 'setting' + '.ini'):
            setting_name = 'setting' + '.ini'
            fi = open(setting_name, "w")
            fi.write('[MusicPlayer]\n')
            # print(os.getcwd())
            fi.write('path =' + os.getcwd())
            fi.close()
